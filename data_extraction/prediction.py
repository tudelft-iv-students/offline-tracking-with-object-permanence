...
class PredictHelper_occ:
    ...
    def _iterate(self, starting_annotation: Dict[str, Any], seconds: float, direction: str, add_time=False) -> List[Dict[str, Any]]:
        """
        ...
        while time_elapsed <= seconds_with_buffer and len(annotations) < max_annotations:
            ...
            if time_elapsed < seconds_with_buffer:
                if add_time:
                    next_annotation['relative_time'] = time_elapsed
    ...
    def _get_past_or_future_for_agent(self, instance_token: str, sample_token: str,
                                      ...
                                      direction: str,
                                      just_xy: bool = True, add_yaw_and_time=False) -> Union[List[Record], np.ndarray]:
        """
        ...
        sequence = self._iterate(starting_annotation, seconds, direction, add_time=add_yaw_and_time)
        ...
        if add_yaw_and_time:
            global_yaw = np.array([r['rotation'] for r in sequence])
            time = np.array([r['relative_time'] for r in sequence])
        if coords.size == 0:
            return coords
    ...
    def get_future_for_agent(self, instance_token: str, sample_token: str,
        ...
        return self._get_past_or_future_for_agent(instance_token, sample_token, seconds,
                                                  in_agent_frame, direction='next', just_xy=just_xy, add_yaw_and_time=add_yaw_and_time)

    def get_past_for_agent(self, instance_token: str, sample_token: str,
                           seconds: float, in_agent_frame: bool, add_yaw_and_time=False,
        ...
        """
        return self._get_past_or_future_for_agent(instance_token, sample_token, seconds,
                                                  in_agent_frame, direction='prev', just_xy=just_xy, add_yaw_and_time=add_yaw_and_time)

    def _get_past_or_future_for_sample(self, sample_token: str, seconds: float, in_agent_frame: bool,
                                       direction: str, just_xy: bool, add_yaw_and_time:bool,
        ...
        for annotation in sample_record['anns']:
            ...
            if not add_yaw_and_time:
                sequence = function(annotation_record['instance_token'],
                                    annotation_record['sample_token'],
                                    seconds, in_agent_frame, direction, just_xy=just_xy,add_yaw_and_time=add_yaw_and_time)

                sequences[annotation_record['instance_token']] = sequence
            else:
                try:
                    coords,global_yaw,time = function(annotation_record['instance_token'],
                                                    annotation_record['sample_token'],
                                                    seconds, in_agent_frame, direction, just_xy=just_xy,add_yaw_and_time=add_yaw_and_time)
                    sequences[annotation_record['instance_token']] = {'coords':coords,
                                                                    'global_yaw':global_yaw,
                                                                    'time': time}
                except:
                    sequences[annotation_record['instance_token']] = {'coords':np.empty([0,2]),
                                                                    'global_yaw':np.empty([0]),
                                                                    'time': np.empty([0,4])}
    ...
    def get_future_for_sample(self, sample_token: str, seconds: float, in_agent_frame: bool,
                              just_xy: bool = True, add_yaw_and_time=False) -> Union[Dict[str, np.ndarray], Dict[str, List[Record]]]:
        """
...
def acceleration(current: Dict[str, Any], prev: Dict[str, Any],
                 time_diff: float, instance_token_for_velocity: str, helper: PredictHelper_occ) -> float:
    """
